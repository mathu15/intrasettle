...
*/
    ...
    issuanceService.getforeigncentralbankaccounts().then((data) => {
      ...
      var yy = xx.filter((pp) => {
        if (pp.organization) return true;
      });
  ...
  const sendasset = async () => {
    ...
    if (selectedasset.length > 0) {
      ...
      ret = await issuanceService.sendcentraltocentral(
        selectedasset[0],
  ...
  const pageDisplay = () => {
    if (activeIndex === 0) {
      ...
      return (
        <SelectParticipantCBforeign
          data={data}
          setData={setData}
          subscribers={subscribers}
          foreigncentralbanks={foreigncentralbanks}
        />
      );
